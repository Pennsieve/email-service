...
data "aws_iam_policy_document" "queue_lambda_iam_policy_document" {
  ...
  statement {
    ...
    resources = [
      ...
      "${aws_dynamodb_table.email_suppression_table.arn}/*",
      aws_dynamodb_table.email_rate_counter_table.arn,
      "${aws_dynamodb_table.email_rate_counter_table.arn}/*",
    ]
  }
...
# Bounce Lambda
############################################################
resource "aws_iam_role" "bounce_lambda_role" {
  name = "${var.environment_name}-${var.service_name}-bounce-lambda-role-${data.terraform_remote_state.region.outputs.aws_region_shortname}"
...
resource "aws_iam_role_policy_attachment" "bounce_lambda_iam_policy_attachment" {
  role       = aws_iam_role.bounce_lambda_role.name
  policy_arn = aws_iam_policy.bounce_lambda_iam_policy.arn
}

resource "aws_iam_policy" "bounce_lambda_iam_policy" {
  name   = "${var.environment_name}-${var.service_name}-bounce-lambda-iam-policy-${data.terraform_remote_state.region.outputs.aws_region_shortname}"
  path   = "/"
  policy = data.aws_iam_policy_document.bounce_lambda_iam_policy_document.json
}

# The bounce lambda only needs to write the suppression table; it is triggered by
# SNS (no queue polling, no SES send).
data "aws_iam_policy_document" "bounce_lambda_iam_policy_document" {
  statement {
    sid    = "BounceLambdaLogsPermissions"
    effect = "Allow"
    actions = [
      "logs:CreateLogGroup",
      "logs:CreateLogStream",
  ...
  # Add bounced/complained addresses to the suppression table.
  statement {
    sid    = "BounceLambdaSuppressionWrite"
    effect = "Allow"
    actions = [
      "dynamodb:GetItem",
      "dynamodb:PutItem",
